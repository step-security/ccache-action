...
import { cacheDir, validateSubscription } from "./common";
...
export class Package {
  ...
  async downloadAndExtract(srcFile: string, dstFile: string) {
    ...
    if (!url.startsWith("https://")) {
      throw new Error(`Refusing to download package from non-HTTPS URL: ${url}`);
    }
    ...
    if (url.endsWith(".zip")) {
      ...
      await tarExtractToFile(dlName, srcFile, dstFile);
    }
  }
  ...
  async installBinary(): Promise<void> {
    ...
    await execArgs("chmod", ["+x", binPath]);
  }
  ...
  async installPackageManager(): Promise<boolean> {
    ...
    let updateCmd: string[] | undefined
    ...
    switch (this.platform) {
      ...
      case PLATFORM.LINUX:
        if (await io.which("apt-get")) {
          ...
          await execArgs(installCmd[0], [...installCmd.slice(1), "epel-release"])
    ...
    const runner = needSudo ? execArgsSudo : execArgs

    try {
      if (shouldUpdate && updateCmd) {
        await runner(updateCmd[0], updateCmd.slice(1))
      }
      await runner(installCmd[0], [...installCmd.slice(1), pkg])
    } catch (error) {
...
async function configure(ccacheVariant: string, platform: string): Promise<void> {
  ...
  if (ccacheVariant === "ccache") {
    await execArgs("ccache", [`--set-config=cache_dir=${cacheDir(ccacheVariant)}`]);
    await execArgs("ccache", [`--set-config=max_size=${maxSize}`]);
    await execArgs("ccache", [`--set-config=compression=true`]);
    if (platform === "darwin" || platform === "win32") {
      // On Windows mtime will be different depending on the Visual Studio installation time, making it unreliable.
      await execArgs("ccache", [`--set-config=compiler_check=content`]);
    }
    if (core.getBooleanInput("create-symlink")) {
      const ccache = await io.which("ccache");
      for (const link of ["gcc", "g++", "cc", "c++", "clang", "clang++", "emcc", "em++"]) {
        await execArgs("ln", ["-s", ccache, `/usr/local/bin/${link}`]);
      }
    }
    core.info("Ccache config:");
    await execArgs("ccache", ["-p"]);
  } else {
    await execArgs("sccache", ["--start-server"], {
      env: {
        ...process.env,
        SCCACHE_IDLE_TIMEOUT: "0",
        SCCACHE_DIR: cacheDir(ccacheVariant),
        SCCACHE_CACHE_SIZE: maxSize,
      },
    });
  }
}

async function execArgs(cmd: string, args: string[], options?: exec.ExecOptions): Promise<void> {
  await exec.exec(cmd, args, options);
}

async function execArgsSudo(cmd: string, args: string[]): Promise<void> {
  // if no sudo is available we are probably in a docker container, and don't need it anyways
  const sudo = await io.which("sudo");
  if (sudo) {
    await exec.exec(sudo, [cmd, ...args]);
  } else {
    await exec.exec(cmd, args);
  }
}

async function tarExtractToFile(tarFile: string, srcInTar: string, dstFile: string): Promise<void> {
  const chunks: Buffer[] = [];
  await exec.exec("tar", ["xf", tarFile, "-O", srcInTar], {
    silent: true,
    listeners: {
  ...
  fs.writeFileSync(dstFile, Buffer.concat(chunks));
}
...
async function runInner(): Promise<void> {
  ...
  await execArgs(ccacheVariant, ["-z"]);
  core.endGroup();
}
