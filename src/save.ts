...
import {AgeUnit, validateSubscription} from "./common";
...
async function ccacheIsEmpty(ccacheVariant : string, ccacheKnowsVerbosityFlag : boolean) : Promise<boolean> {
  if (ccacheVariant === "ccache") {
    if (ccacheKnowsVerbosityFlag) {
      return !!(await exec.getExecOutput("ccache", ["-s", "-v"], {silent: true})).stdout.match(/Files:.+\b0\b/);
    } else {
      return !!(await exec.getExecOutput("ccache", ["-s"], {silent: true})).stdout.match(/files in cache.+\b0\b/)
    }
  } else {
    return !!(await exec.getExecOutput("sccache", ["-s"], {silent: true})).stdout.match(/Cache size.+\b0 bytes/);
  }
}

async function getVerbosityArgs(verbositySetting : string) : Promise<string[]> {
...
async function run(earlyExit : boolean | undefined) : Promise<void> {
  ...
  try {
    ...
    const ccacheKnowsVerbosityFlag = !!(await exec.getExecOutput(ccacheVariant, ["--help"], {silent: true})).stdout.includes("--verbose");

    core.startGroup(`${ccacheVariant} stats`);
    const verbosityArgs = ccacheKnowsVerbosityFlag ? await getVerbosityArgs(core.getInput("verbose")) : [];
    await exec.exec(ccacheVariant, ["-s", ...verbosityArgs]);
